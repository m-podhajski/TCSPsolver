...
class LLClosed {
private:
    static WeakLinearOrder create_third_order(const WeakLinearOrder &order1, const WeakLinearOrder &order2, int e) {
...
public:
    static bool check_closure(const TemporalConstraintLanguage &constraint) {
            ...
            for (const auto &order1: relation.tuples) {
                for (const auto &order2: relation.tuples) {
                    for (int e = 0; e < relation.arity; e++) {
                        const WeakLinearOrder & thirdOrder= create_third_order(order1, order2, e);
    ...
    static bool solve(const TemporalConstraintLanguage &constraint){
        return true;
    }
};
